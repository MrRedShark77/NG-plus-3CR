...
function drawTreeBranch(num1, num2, tier=1) {
...
function drawTSName(id,tier) {
    var start = document.getElementById(tier == 1 ? id : "TST"+tier+"_"+id).getBoundingClientRect();
    var x1 = start.left + (start.width / 2) + (document.documentElement.scrollLeft || document.body.scrollLeft);
    var y1 = start.top + (start.height / 2) + (document.documentElement.scrollTop || document.body.scrollTop);
    ctx.fillStyle = 'white';
    ...
    if (tier == 1 && (document.getElementById(id).className.split(" ")[1] !== undefined || id > 220)) {
        var tempName = document.getElementById(id).className.split(" ")[1];
        var name;
        //console.log(id)
        if (id == 222 || id == 223 || id == 226 || id == 227 || id == 232 || id == 233) name = "dark"
        else if (id == 221 || id == 224 || id == 225 || id == 228 || id == 231 || id == 234) name = "light"
        ...
        ctx.strokeText(id+" "+name, x1 - start.width / 2, y1 - start.height / 2 - 1);
        ctx.fillText(id+" "+name, x1 - start.width / 2, y1 - start.height / 2 - 1);
    } else {
        let txt
        if (tier == 1) txt = id
        else {
            let mult = TS_TIERS[tier][id][3]??1
            let row = Math.floor(id/10), col = id%10

            txt = tier+"-"+id
            if ((row==12||row==13) && col <= 3) txt += ['R','G','B'][col-1]
            
            if (mult>1) txt += " (" + (mult>1e3?shorten(mult):mult) + "x)"
            else if (mult <= 0) txt += " (reset)"
        }
